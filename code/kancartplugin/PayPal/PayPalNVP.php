...
class paypal_curl extends base {
    ...
    function SetExpressCheckout($returnUrl, $cancelUrl, $options = array()) {
        ...
        if ($this->_mode == 'payflow') {
            ...
            if (!isset($values['PAYMENTACTION']) || ($this->checkHasApiCredentials() === FALSE))
                $values['PAYMENTACTION'] = ($this->_trxtype == 'S' || ($this->checkHasApiCredentials() === FALSE) ? 'Sale' : 'Authorization');
        ...
        if (defined('MODULE_PAYMENT_PAYPALWPP_HEADER_BORDER_COLOR'))
            $values['HDRBORDERCOLOR'] = MODULE_PAYMENT_PAYPALWPP_HEADER_BORDER_COLOR;
        if (defined('MODULE_PAYMENT_PAYPALWPP_HEADER_BACK_COLOR'))
            $values['HDRBACKCOLOR'] = MODULE_PAYMENT_PAYPALWPP_HEADER_BACK_COLOR;
    ...
    function DoExpressCheckoutPayment($token, $payerId, $options = array()) {
        ...
        KCLogger::Log('DoExpressCheckoutPayment params: ' . json_encode($this->values));
