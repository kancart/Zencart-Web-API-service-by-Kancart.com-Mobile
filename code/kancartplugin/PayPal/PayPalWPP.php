 ...
 */
require_once('kancartplugin/PayPal/PayPalNVP.php');
...
class PayPalWPP extends base {
     ...
     * @var string
     */
    var $buttonSourceEC = 'Kancart_SP_EC';
    ...
    function before_process() {
        ...
        if ($_SESSION['paypal_ec_payer_info']['ship_street_1'] != '' && strtoupper($_SESSION['paypal_ec_payer_info']['ship_address_status']) != 'NONE') {
            $options = array_merge($options, array('PAYMENTREQUEST_0_SHIPTONAME' => $_SESSION['paypal_ec_payer_info']['ship_name'],
        ...
        if (isset($options['PAYMENTREQUEST_0_SHIPTOPHONE']) && trim($options['PAYMENTREQUEST_0_SHIPTOPHONE']) == '')
            unset($options['PAYMENTREQUEST_0_SHIPTOPHONE']);
        ...
        KCLogger::Log('DoExpressCheckoutPayment response: ' . json_encode($response));
        ...
        if (empty($response['PAYMENTINFO_0_PENDINGREASON']) ||
            ...
            if ($response['PAYMENTINFO_0_PAYMENTSTATUS'] == 'Pending') {
                if ($response['PAYMENTINFO_0_L_ERRORCODE0'] == 11610 && $response['PAYMENTINFO_0_PENDINGREASON'] == '')
    ...
    function after_process() {
        ...
        // add kancart order comments
        $commentString = $db->bindVars($commentString, ':kancartComments:', (isset($_REQUEST['custom_kc_comments']) ? $_REQUEST['custom_kc_comments'] : ''), 'noquotestring');
    ...
    function zcLog($stage, $message) {
        ...
        if (MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log and Email' || MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log File') {
            $token = (isset($_SESSION['paypal_ec_token'])) ? $_SESSION['paypal_ec_token'] : preg_replace('/[^0-9.A-Z\-]/', '', $_POST['token']);
    ...
    function getLineItemDetails($restrictedCurrency) {
        ...
        KCLogger::Log('DEFAULT_CURRENCY: ' . DEFAULT_CURRENCY);
        KCLogger::Log('$order currency: ' . $order->info['currency']);
        KCLogger::Log('$restrictedCurrency: ' . $restrictedCurrency);
        KCLogger::Log('SESSION: ' . $_SESSION['currency'] . '.value = ' . $currencies->currencies[$_SESSION['currency']]['value']);
        KCLogger::Log('ORDER: ' . $order->info['currency'] . '.value = ' . $currencies->currencies[$order->info['currency']]['value']);
            ...
            for ($i = 0, $n = sizeof($order_totals); $i < $n; $i++) {
                ...
                if (in_array($order_totals[$i]['code'], array('ot_total', 'ot_subtotal', 'ot_tax', 'ot_shipping')) || strstr($order_totals[$i]['code'], 'insurance')) {
                    ...
                    if ($order_totals[$i]['code'] == 'ot_total') {
                        ...
                        KCLogger::Log('set payment amount: ' . $optionsST['PAYMENTREQUEST_0_AMT']);
                    }
                    ...
                    //$optionsST['PAYMENTREQUEST_0_SHIPDISCAMT'] = '';  // Not applicable
                } else {
        ...
        for ($i = 0, $n = sizeof($order->products), $k = 0; $i < $n; $i++, $k++) {
            ...
            if (isset($order->products[$i]['attributes']) && sizeof($order->products[$i]['attributes']) > 0) {
                $optionsLI["L_PAYMENTREQUEST_0_DESC$k"] = '';
            ...
            if ($flag_treat_as_partial || $order->products[$i]['qty'] != (int) $order->products[$i]['qty']) {
                ...
                $optionsLI["L_PAYMENTREQUEST_0_AMT$k"] = zen_round($order->products[$i]['qty'] * $order->products[$i]['final_price'], $currencies->currencies[$_SESSION['currency']]['decimal_places']);
                $optionsLI["L_PAYMENTREQUEST_0_TAXAMT$k"] = zen_calculate_tax(zen_round($order->products[$i]['qty'] * $order->products[$i]['final_price'], $currencies->currencies[$_SESSION['currency']]['decimal_places']), $order->products[$i]['tax']);
                $optionsLI["L_PAYMENTREQUEST_0_QTY$k"] = 1;
            } else {
                $optionsLI["L_PAYMENTREQUEST_0_AMT$k"] = $order->products[$i]['final_price'];
                $optionsLI["L_PAYMENTREQUEST_0_QTY$k"] = $order->products[$i]['qty'];
                $optionsLI["L_PAYMENTREQUEST_0_TAXAMT$k"] = zen_calculate_tax(1 * $order->products[$i]['final_price'], $order->products[$i]['tax']);
            }

            // For tax-included pricing, combine tax with price instead of treating separately:
            if (DISPLAY_PRICE_WITH_TAX == 'true') {
                $optionsLI["L_PAYMENTREQUEST_0_AMT$k"] += $optionsLI["L_PAYMENTREQUEST_0_TAXAMT$k"];
                $optionsLI["L_PAYMENTREQUEST_0_TAXAMT$k"] = 0;
            }
            $subTotalLI += ($optionsLI["L_PAYMENTREQUEST_0_QTY$k"] * $optionsLI["L_PAYMENTREQUEST_0_AMT$k"]);
            $subTotalTax += ($optionsLI["L_PAYMENTREQUEST_0_QTY$k"] * $optionsLI["L_PAYMENTREQUEST_0_TAXAMT$k"]);
            ...
            if ($order->products[$i]['onetime_charges'] != 0) {
                ...
                $optionsLI["L_PAYMENTREQUEST_0_AMT$k"] = $order->products[$i]['onetime_charges'];
                $optionsLI["L_PAYMENTREQUEST_0_QTY$k"] = 1;
                $optionsLI["L_PAYMENTREQUEST_0_TAXAMT$k"] = zen_calculate_tax($order->products[$i]['onetime_charges'], $order->products[$i]['tax']);
        ...
        if ($surcharges > 0) {
            ...
            $optionsLI["L_PAYMENTREQUEST_0_NAME$k"] = MODULES_PAYMENT_PAYPALWPP_LINEITEM_TEXT_SURCHARGES_SHORT;
            $optionsLI["L_PAYMENTREQUEST_0_DESC$k"] = MODULES_PAYMENT_PAYPALWPP_LINEITEM_TEXT_SURCHARGES_LONG;
        ...
        if ($creditsApplied > 0) {
            $numberOfLineItemsProcessed++;
            $k = $numberOfLineItemsProcessed;
            $optionsLI["L_PAYMENTREQUEST_0_NAME$k"] = MODULES_PAYMENT_PAYPALWPP_LINEITEM_TEXT_DISCOUNTS_SHORT;
            $optionsLI["L_PAYMENTREQUEST_0_DESC$k"] = MODULES_PAYMENT_PAYPALWPP_LINEITEM_TEXT_DISCOUNTS_LONG;
        ...
        for ($k = 0, $n = $numberOfLineItemsProcessed + 1; $k < $n; $k++) {
            ...
            if (isset($optionsLI["L_PAYMENTREQUEST_0_DESC$k"]))
                $optionsLI["L_PAYMENTREQUEST_0_DESC$k"] = str_replace(array('&', '=', '%'), '*', $optionsLI["L_PAYMENTREQUEST_0_DESC$k"]);
            ...
            if (isset($optionsLI["L_PAYMENTREQUEST_0_DESC$k"]))
                $optionsLI["L_PAYMENTREQUEST_0_DESC$k"] = zen_clean_html($optionsLI["L_PAYMENTREQUEST_0_DESC$k"], 'strong');
            ...
            if (isset($optionsLI["L_PAYMENTREQUEST_0_DESC$k"]) && $optionsLI["L_PAYMENTREQUEST_0_DESC$k"] == '')
                unset($optionsLI["L_PAYMENTREQUEST_0_DESC$k"]);
            if (isset($optionsLI["L_PAYMENTREQUEST_0_DESC$k"]))
                $optionsLI["L_PAYMENTREQUEST_0_DESC$k"] = substr($optionsLI["L_PAYMENTREQUEST_0_DESC$k"], 0, 127);
        ...
        $stDiffTaxOnly = (strval($sumOfLineItems - $sumOfLineTax - round($optionsST['PAYMENTREQUEST_0_AMT'], 2)) + 0);
        if (DISPLAY_PRICE_WITH_TAX == 'true' && $stDiffTaxOnly == 0) {
            $optionsNB["PAYMENTREQUEST_0_DESC"] = 'Tax included in prices: ' . $sumOfLineTax . ' (' . $optionsST['PAYMENTREQUEST_0_TAXAMT'] . ') ';
            $optionsST['PAYMENTREQUEST_0_TAXAMT'] = 0;
            for ($k = 0, $n = $numberOfLineItemsProcessed + 1; $k < $n; $k++) {
        ...
        if (strval($optionsST['PAYMENTREQUEST_0_ITEMAMT']) < 0) {
            ...
            if ($optionsST['PAYMENTREQUEST_0_AMT'] < $optionsST['PAYMENTREQUEST_0_TAXAMT'])
                $optionsST['PAYMENTREQUEST_0_TAXAMT'] = 0;
            if ($optionsST['PAYMENTREQUEST_0_AMT'] < $optionsST['PAYMENTREQUEST_0_SHIPPINGAMT'])
                $optionsST['PAYMENTREQUEST_0_SHIPPINGAMT'] = 0;
            $discountProblemsFlag = TRUE;
            ...
            if (is_array($optionsLI))
                foreach ($optionsLI as $key => $value) {
                    if (substr($key, 0, 8) == 'L_TAXAMT' && ($optionsLI[$key] == '' || $optionsLI[$key] == 0)) {
                        ...
                        if (strstr($key, 'PAYMENTREQUEST_0_AMT'))
        ...
        KCLogger::Log(' getLineItemDetails success: ' . json_encode(array_merge($optionsST, $optionsLI, $optionsNB)));
        // Send Subtotal and LineItem results back to be submitted to PayPal
        return array_merge($optionsST, $optionsLI, $optionsNB);
    }

    // TODO SetExpressCheckout
    function doSetExpressCheckout() {
        ...
        if ($lc_code != '')
            $options['PAYMENTREQUEST_0_LOCALECODE'] = $lc_code;
        ...
        $cancel_url = urldecode($_POST['cancel_url']);
        ...
        KCLogger::Log('$requestParams: ' . json_encode($requestParams));
         ...
         */
        $results = $this->_errorHandler($response, 'SetExpressCheckout');
        ...
        if (count($results) == 0) {
            $results['token'] = $_SESSION['paypal_ec_token'];
            $results['paypal_redirect_url'] = $paypal_url . "?cmd=_express-checkout&token=" . $_SESSION['paypal_ec_token'] . $userActionKey;
        }
        return $results;
    }

    // TODO getExpressCheckoutDetail
    function doGetExpressCheckoutDetail() {
        ...
        if (empty($_SESSION['paypal_ec_token'])) {
            ...
            if (isset($_POST['token'])) {
                ...
                $_SESSION['paypal_ec_token'] = $_POST['token'];
                // sanitize this
                $_SESSION['paypal_ec_token'] = preg_replace('/[^0-9.A-Z\-]/', '', $_POST['token']);
            } else {
                // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        ...
        if (MODULE_PAYMENT_PAYPALWPP_CONFIRMED_ADDRESS == 'Yes' && strtoupper($response['PAYMENTREQUEST_0_ADDRESSSTATUS']) != 'CONFIRMED') {
            // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        ...
        $gender = '';
        if (urldecode($response['SALUTATION'] == 'Mr'))
            $gender = 'm';
        if (in_array(urldecode($response['SALUTATION']), array('Ms', 'Mrs')))
            $gender = 'f';
        ...
        $step2_payerinfo = array('payer_id' => $response['PAYERID'],
            ...
            'order_comment' => urldecode($response['PAYMENTREQUEST_0_NOTETEXT']),
        );

//    if (strtoupper($response['PAYMENTREQUEST_0_ADDRESSSTATUS']) == 'NONE' || !isset($response['PAYMENTREQUEST_0_SHIPTOSTREET']) || $response['PAYMENTREQUEST_0_SHIPTOSTREET'] == '') {
//      $step2_shipto = array();
//    } else {
        ...
        $step2_shipto = array('ship_name' => urldecode($response['PAYMENTREQUEST_0_SHIPTONAME']),
            ...
            'ship_country_name' => (isset($response['PAYMENTREQUEST_0_SHIPTOCOUNTRY']) ? urldecode($response['PAYMENTREQUEST_0_SHIPTOCOUNTRY']) : urldecode($response['PAYMENTREQUEST_0_SHIPTOCOUNTRYNAME'])));
//    }
        ...
        if (!(isset($_SESSION['paypal_ec_markflow']) && $_SESSION['paypal_ec_markflow'] == 1)) {
            KCLogger::Log('+++++++++++++ unset($_SESSION[\'shipping\']');
        ...
        global $order;
        $order->info['total'] = $response['PAYMENTREQUEST_0_AMT'];
        ...
        $this->doGetExpressCheckoutDetail_finish(array_merge($step2_payerinfo, $step2_shipto), $this->new_acct_notify);
    }

    // TODO doGetExpressCheckoutDetail_finish
    function doGetExpressCheckoutDetail_finish($paypal_ec_payer_info, $new_acct_notify) {
        global $db, $order;
        ...
        if (!empty($_SESSION['customer_first_name']) && !empty($_SESSION['customer_id']) && $_SESSION['customer_id'] > 0) {
            ...
            if (!$check_customer->EOF) {
                ...
                if ($check_customer->fields['customers_paypal_ec'] == '1') {
                    ...
                    KCLogger::Log('Delete the existing temporary account: ' . $check_customer->fields['customers_id']);
    ...
    function setShippingMethod() {
        ...
        if (!$_SESSION['shipping'] || ( $_SESSION['shipping'] && ($_SESSION['shipping'] == false) && (zen_count_shipping_modules() > 1) )) {
            ...
            KCLogger::Log('PayPal WPP setShippingMethod: ' . json_encode($_SESSION['shipping']));
        }
        KCLogger::Log('PayPal WPP setShippingMethod done');
    }
...
///        $fromDb = str_replace("\n", '', $answers_arr->fields['entry_firstname'].$answers_arr->fields['entry_lastname']);
    ...
    function user_login($email_address, $redirect = true) {
        ...
        if ($session_started == false) {
            die('zen_redirect die1');
            zen_redirect(zen_href_link(FILENAME_COOKIE_USAGE));
        }
        $i = 0;
        KCLogger::Log('user_login: ' . $i);
        $i++;
    ...
    function terminateEC($error_msg = '', $kill_sess_vars = false, $goto_page = '') {
        global $messageStack, $order, $order_total_modules, $kcResponse;
        ...
        if ($kill_sess_vars) {
            if (!empty($_SESSION['paypal_ec_temp'])) {
                KCLogger::Log('Delete the existing temporary account: ' . $_SESSION['customer_id']);
        ...
        KCLogger::Log('terminateEC: ' . json_encode(array('redirect_to_page' => $redirect_path, 'messages' => $error_msg)));


        $kcResponse->DataBack(array('redirect_to_page' => $redirect_path, 'messages' => array($error_msg)));
    }
    ...
    function _errorHandler($response, $operation = '', $ignore_codes = '') {
        global $messageStack, $doPayPal, $kcResponse;
        $results = array();
        ...
        switch ($operation) {
            case 'SetExpressCheckout':
                if ($basicError) {
                    ...
                    array_merge($results, $this->terminateEC($errorText . ' (' . $errorNum . ') ' . $detailedMessage, true));
                    return $results;
                }
                break;
            ...
            case 'DoExpressCheckoutPayment':
                if ($basicError || $_SESSION['paypal_ec_token'] != urldecode($response['TOKEN'])) {
                    ...
                    if ($response['L_ERRORCODE0'] == 10422) {
                        $paypal_url = $this->getPayPalLoginServer();
                        $kcResponse->DataBack(array('paypal_redirect_url' => $paypal_url . "?cmd=_express-checkout&token=" . $_SESSION['paypal_ec_token'], 'token' => $_SESSION['paypal_ec_token']));
                    }
